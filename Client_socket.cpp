...
int main(){
    ...
    int r = connect(client_socket, (SOCKADDR*)&addr, sizeof(addr));
    if(r < 0){
        cout<<"client connection error"<<endl;
        system("pause");
        int c =closesocket(client_socket);
    }
    else{
        ...
        while(1){
            ...
            if(whosturn==2){
                ...
                if(end_check == 0){
                    check_connect[0]='1';
                    send(client_socket, check_connect, (int)strlen(check_connect), 0);
                    check_connect[0]='0';
                    //將字串keyin傳給server端
                    send(client_socket, keyin, (int)strlen(keyin), 0);
                    goto no_winner;
                }
                //有人贏了
                else{
            ...
            //server端的回合
                ...
                recv(client_socket, check_connect, (int)strlen(check_connect), 0);
                if(check_connect[0]=='1'){
                    check_connect[0]='0';
                    ZeroMemory(keyin, 100);
                    //將從server端收到的字串存到keyin
                    recv(client_socket, keyin, sizeof(keyin), 0);
                    ...
                    system("cls");
                    show_board();
                    cout<<"Server's connection break!";
                    break;
                }
            }
no_winner:
